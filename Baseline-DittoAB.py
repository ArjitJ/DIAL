...
unpaireddataset = MyDataset(dataDir, indices, tokenizer, datasetColumns)
...
optimizer = AdamW([{'params': model.transformer.parameters(), 'lr':3e-5}, {'params':model.fc.parameters()},
...
for epoch in range(numEpochs):
    for (x, y, label) in unpaired_loader:
        ...
        for i in range(numEmbeddings):
            u = embeddings_x[i]
            v = embeddings_y[i]
            classification_loss = classification_loss + CE(model.fin[i](torch.cat([u, v, torch.abs(u-v)], -1)), label)
        loss = classification_loss/numEmbeddings
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        scheduler.step()
